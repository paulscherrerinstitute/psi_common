...
begin
  ...
  begin
    ...
    if num_reg_g > 0 then
      ...
      axi_apply_wd_burst(3, 1, 1, "1111", "1111", axi_ms, axi_sm, s_axi_aclk, axi_throttling_g);
      ...
      axi_expect_rresp_burst(3, 16#100#, 1, xRESP_OKAY_c, axi_ms, axi_sm, s_axi_aclk, false, false, axi_throttling_g);
      WaitDone(3);
    end if;
    ...
    if num_reg_g > 0 then
      ...
      TestCase <= 4;
      -- write
      axi_apply_aw(0, AxSIZE_4_c, num_reg_g - 1, xBURST_INCR_c, axi_ms, axi_sm, s_axi_aclk);
      axi_apply_wd_burst(num_reg_g, 10, 1, "1111", "1111", axi_ms, axi_sm, s_axi_aclk, axi_throttling_g);
      axi_expect_bresp(xRESP_OKAY_c, axi_ms, axi_sm, s_axi_aclk);
      -- read
      axi_apply_ar(0, AxSIZE_4_c, num_reg_g - 1, xBURST_INCR_c, axi_ms, axi_sm, s_axi_aclk);
      axi_expect_rresp_burst(num_reg_g, 16#1000#, 1, xRESP_OKAY_c, axi_ms, axi_sm, s_axi_aclk, false, false, axi_throttling_g);
      WaitDone(4);
    end if;
    ...
    TestCase <= 5;
    -- write
    axi_apply_aw(4 * (num_reg_g + 1), AxSIZE_4_c, 4 - 1, xBURST_INCR_c, axi_ms, axi_sm, s_axi_aclk);
    axi_apply_wd_burst(4, 16#200#, 1, "1100", "0111", axi_ms, axi_sm, s_axi_aclk, axi_throttling_g);
    if use_mem_g then
      axi_expect_bresp(xRESP_OKAY_c, axi_ms, axi_sm, s_axi_aclk);
    else
    ...
    -- read
    axi_apply_ar(4 * (num_reg_g + 2), AxSIZE_4_c, 4 - 1, xBURST_INCR_c, axi_ms, axi_sm, s_axi_aclk);
    if use_mem_g then
      axi_expect_rresp_burst(4, 16#302#, 1, xRESP_OKAY_c, axi_ms, axi_sm, s_axi_aclk, false, false, axi_throttling_g);
    else
      axi_expect_rresp_burst(4, 16#302#, 1, xRESP_DECERR_c, axi_ms, axi_sm, s_axi_aclk, true, false, axi_throttling_g);
    end if;
    WaitDone(5);
    ...
    TestCase <= 6;
    -- write
    axi_apply_aw(4 * (num_reg_g - 2), AxSIZE_4_c, 4 - 1, xBURST_INCR_c, axi_ms, axi_sm, s_axi_aclk);
    axi_apply_wd_burst(4, 16#400#, 1, "1111", "1111", axi_ms, axi_sm, s_axi_aclk, axi_throttling_g);
    if use_mem_g then
      axi_expect_bresp(xRESP_OKAY_c, axi_ms, axi_sm, s_axi_aclk);
    else
      -- Expect error if memory interface is not implemented
    ...
    axi_apply_ar(4 * (num_reg_g - 2), AxSIZE_4_c, 4 - 1, xBURST_INCR_c, axi_ms, axi_sm, s_axi_aclk);
    if use_mem_g then
      axi_expect_rresp_burst(4, 16#500#, 1, xRESP_OKAY_c, axi_ms, axi_sm, s_axi_aclk, false, false, axi_throttling_g);
    else
      axi_expect_rresp_burst(4, 16#500#, 1, xRESP_DECERR_c, axi_ms, axi_sm, s_axi_aclk, true, false, axi_throttling_g);
    end if;
    WaitDone(6);
  ...
  begin
    ...
    if use_mem_g then
      ...
      while RecWords_v(3 downto 0) /= "1111" loop
        ...
        if o_reg_wr(num_reg_g - 2) = '1' then
          StdlvCompareInt(16#400#, o_reg_wdata(num_reg_g - 2), "Wrong reset data[0]");
          RecWords_v(0) := '1';
        end if;
        if o_reg_wr(num_reg_g - 1) = '1' then
          StdlvCompareInt(16#401#, o_reg_wdata(num_reg_g - 1), "Wrong reset data[1]");
          RecWords_v(1) := '1';
        end if;
    ...
    -- read
    if use_mem_g then
      i_reg_rdata(num_reg_g - 2) <= X"00000500";
      i_reg_rdata(num_reg_g - 1) <= X"00000501";
      RecWords_v                := (others => '0');
      StartTime_v               := now;
      while now < StartTime_v + 1 us loop
        wait until rising_edge(s_axi_aclk);
